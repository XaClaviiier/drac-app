...
import { Bell, Bot, Boxes, Building2, Car, ChevronDown, CirclePlus, FileText, Grid2X2, Home, PackagePlus, Settings, Sparkles, Users, Warehouse, Wrench, X, Truck, FolderTree, ReceiptText, Shield } from 'lucide-react';
import { useApp } from '../context/AppContext';

export default function MobileDashboard(){
  const {data,currentUser,currentBranchId,setCurrentBranchId,hasPermission}=useApp();
  const navigate=useNavigate(); const [branchesOpen,setBranchesOpen]=useState(false); const [moreOpen,setMoreOpen]=useState(false); const [addOpen,setAddOpen]=useState(false);
  ...
  return <div className="min-h-screen bg-[#031a35] pb-24 text-white lg:hidden">
    ...
    <div className="relative mx-auto max-w-md px-4 pb-6 pt-[max(18px,env(safe-area-inset-top))]">
      <header className="flex items-center gap-3">
        ...
        <div className="relative flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-violet-400 to-purple-700 font-bold">{currentUser?.name?.[0]}<span className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-[#031a35] bg-emerald-400"/></div>
      </header>

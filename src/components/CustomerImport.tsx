...
export default function CustomerImport({ branches, defaultBranchId, onClose, onComplete, embedded = false }: Props) {
  ...
  const report = () => {
    ...
    downloadImportCsv('laporan-import-customer.csv', [['runId', 'source', 'branchId', 'executionState', 'executionError', ...fields], ...preview.rows.map(row => [attempt?.runId, source, branchId, preview.verified ? 'verified' : uncertain ? 'unverified' : 'preview_only', uncertain ? error : '', ...fields.map(field => uncertain && row.status === 'new' && field === 'status' ? 'unverified' : row[field])])]);
  };
  ...
  const content = <div className={`${embedded ? 'mx-auto max-w-6xl' : 'mx-auto my-2 max-h-[calc(100vh-1rem)] max-w-6xl overflow-y-auto sm:my-6 sm:max-h-[calc(100vh-3rem)]'} space-y-4 rounded-xl bg-white p-4 sm:p-6`}>
      ...
      <div className="flex flex-wrap items-end gap-3">
        <label className="min-w-0 text-sm">File CSV / XLSX (maks. 8 MB, 5.000 baris)<input aria-label="File customer" type="file" accept=".csv,.xlsx" disabled={locked} onChange={event => void load(event.target.files?.[0])} className="mt-1 block w-full max-w-xs text-sm" /></label>
        <button className={button} onClick={() => downloadImportCsv('template-customer.csv', [['name','phone','customer_category','accurate_id','email','address','business_phone','contact','description'], ['PELANGGAN CONTOH','081234567890','Umum','','','','','','']])}>Unduh template</button>
        <label className="text-sm">Cabang pertama input<select aria-label="Cabang pertama input" value={branchId} disabled={locked} onChange={event => { setBranchId(event.target.value); invalidate(); }} className="ml-2 rounded border p-2"><option value="">Pilih cabang</option>{branches.map(branch => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label>
      </div>
      ...
      {preview && <>
        ...
        {preview.verified && <p role="status" className="rounded bg-green-50 p-3 text-green-800">Selesai. {preview.counts.created} customer tersimpan dan lolos verifikasi baca ulang. Baris lain tidak mengubah master.</p>}
        <div className="flex flex-wrap gap-3"><label className="text-sm">Status <select aria-label="Filter status import" value={filter} onChange={event => { setFilter(event.target.value); setPage(0); }} className="rounded border p-2"><option value="all">Semua</option>{Object.entries(statuses).map(([key,label]) => <option key={key} value={key}>{label}</option>)}</select></label><button className={button} onClick={report}>Unduh laporan per baris</button></div>

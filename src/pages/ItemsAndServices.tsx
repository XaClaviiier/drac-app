...
export default function ItemsAndServices() {
  ...
  const deletePendingItem = async (itemId: string) => {
    const item = pendingItems.find(row => row.id === itemId);
    if (!item || !window.confirm(`Hapus barang Pending ${item.code} — ${item.name}?\n\nTindakan ini hanya berhasil jika barang belum dipakai dalam penerimaan/transaksi dan tidak memiliki stok.`)) return;
    setVerifyingItemId(itemId);
    setVerificationFeedback({ type: 'info', message: 'Sedang memeriksa dan menghapus barang Pending…' });
    try {
      const response = await api.remove('items', itemId);
      if (!response.success) throw new Error(response.message || 'Barang Pending gagal dihapus.');
      await refreshData();
      setMergeTargets(current => { const next = { ...current }; delete next[itemId]; return next; });
      setVerificationFeedback({ type: 'success', message: response.message || 'Barang Pending berhasil dihapus.' });
    } catch (error: any) {
      setVerificationFeedback({ type: 'error', message: error?.message || 'Barang Pending gagal dihapus.' });
    } finally {
      setVerifyingItemId('');
    }
  };
    ...
    <div className="space-y-0">
      ...
      {itemListTab === 'verification' && <section className="min-h-[560px] border border-slate-300 bg-[#eeeeee] p-4">
        ...
        <div className="overflow-x-auto rounded-t-lg border border-slate-300 bg-white">
          <div className="grid min-w-[1140px] grid-cols-[160px_minmax(240px,1fr)_170px_minmax(280px,1fr)_300px] bg-[#637c93] px-3 py-2.5 text-sm font-semibold text-white"><span>Kode Barang</span><span>Nama Barang</span><span>Kategori / Stok</span><span>Master Tujuan</span><span>Aksi</span></div>
          {pendingItems.map(item => <div key={item.id} className="grid min-w-[1140px] grid-cols-[160px_minmax(240px,1fr)_170px_minmax(280px,1fr)_300px] items-center border-b border-slate-300 px-3 py-2 text-sm odd:bg-white even:bg-slate-50">

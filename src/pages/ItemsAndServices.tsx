                ...
                <div className="grid gap-8 lg:grid-cols-2 lg:gap-10 xl:gap-12">
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-[170px_minmax(0,1fr)] items-center gap-x-4 gap-y-3 text-sm">
                      ...
                      <div className="flex gap-2"><select disabled={itemForm.type==='Jasa'||itemForm.type==='Group'} value={itemForm.itemBrandId} onChange={e=>{const selected=itemBrands.find(row=>row.id===e.target.value);setItemForm({...itemForm,itemBrandId:e.target.value,brand:selected?.name||''});}} className="h-9 min-w-0 flex-1 rounded border border-slate-300 bg-white px-2 disabled:bg-slate-100"><option value="">Tanpa merek</option>{itemBrands.filter(row=>row.isActive||row.id===itemForm.itemBrandId).map(row=><option key={row.id} value={row.id}>{row.code} - {row.name}</option>)}</select><button type="button" onClick={()=>setShowBrandModal(true)} title="Kelola merek barang" aria-label="Kelola merek barang" className="flex h-9 w-10 items-center justify-center rounded border border-blue-500 text-blue-700"><Edit className="h-4 w-4"/></button></div>

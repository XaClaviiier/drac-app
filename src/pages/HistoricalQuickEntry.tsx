...
export default function HistoricalQuickEntry(){
  ...
  const [groqKey,setGroqKey]=useState(''); const [groqModel,setGroqModel]=useState('llama-3.3-70b-versatile'); const [savingKey,setSavingKey]=useState(false); const [aiConfigured,setAiConfigured]=useState(false);
  useEffect(()=>{ api.get<AccountMap[]>('branch-account-settings').then(r=>setAccountMaps(r.data||[])); },[]);
  useEffect(()=>{ api.getAISettings().then(r=>{if(r.success&&r.data){setAiConfigured(!!r.data.configured);setGroqModel(r.data.model||'llama-3.3-70b-versatile');}}); },[]);
  ...
  const saveGroqKey=async()=>{
    ...
    setSavingKey(true); const r=await api.updateAISettings(clean,groqModel); setSavingKey(false);
  ...
  return <div className="mx-auto max-w-7xl p-4">
    ...
    <div className="rounded-xl border bg-white p-4 shadow-sm">
      ...
      {currentUser?.isOwner&&<div className="mb-4 rounded-xl border border-violet-200 bg-violet-50 p-3"><div className="mb-2 flex items-center justify-between gap-2"><div><div className="flex items-center gap-2 font-semibold text-violet-900"><KeyRound className="h-4 w-4"/>Groq Key Pembaca Nota</div><p className="text-xs text-violet-700">{aiConfigured?'Key perusahaan sudah aktif. Tempel key baru hanya saat key lama habis atau tidak aktif.':'Belum ada key aktif. Tempel key dari console.groq.com.'}</p></div><span className={`rounded-full px-2 py-1 text-xs font-semibold ${aiConfigured?'bg-green-100 text-green-700':'bg-amber-100 text-amber-700'}`}>{aiConfigured?'Aktif':'Belum aktif'}</span></div><div className="flex flex-col gap-2 sm:flex-row"><input type="password" autoComplete="new-password" value={groqKey} onChange={e=>setGroqKey(e.target.value)} placeholder="Tempel Groq API Key: gsk_..." className="min-w-0 flex-1 rounded-lg border border-violet-200 bg-white px-3 py-2 font-mono text-sm"/><button type="button" disabled={savingKey||!groqKey.trim()} onClick={()=>void saveGroqKey()} className="rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">{savingKey?'Menyimpan...':aiConfigured?'Ganti Key':'Simpan Key'}</button></div><p className="mt-1.5 text-xs text-slate-500">Key disimpan terenkripsi di server dan tidak ditampilkan kembali.</p></div>}

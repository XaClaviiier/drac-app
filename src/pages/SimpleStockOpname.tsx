...
export default function SimpleStockOpname({legacy}:{legacy:ReactNode}) {
  ...
  const clear=()=>{setDocument(null);setRows([]);setDate(localDateKey());setWarehouse('');setNotes('');setDirty(false);setSearch('');setFilter('all');setRequestKey(key());setInfo(false);setImportOpen(false);setMessage('');};
  ...
  const open=async(id:string)=>{
    ...
    const doc=result.data;setDocument(doc);setRows(normalize(doc.rows||[]));setWarehouse(doc.warehouseId);setDate(doc.endDate);setNotes(doc.notes||'');setDirty(false);setTab(true);setList(false);setInfo(false);setSearch('');setFilter('all');
  };
  const preview=async(nextWarehouse:string,nextDate:string,preserve=false)=>{
    if(!nextWarehouse){setRows([]);return;}
    ...
    setRows(normalize(result.data.rows).map(row=>preserve?{...row,count:rows.find(old=>old.itemId===row.itemId)?.count??''}:row));
  };
  ...
  const reload=()=>void run(async()=>{
    ...
    if(document){
      ...
      const fresh=result.data.rows||[];setRows(rows.map(row=>({...row,editVersion:fresh.find(x=>x.itemId===row.itemId)?.editVersion??row.editVersion})));
  ...
  const template=()=>void run(async()=>{
    ...
    rows.forEach(row=>sheet.addRow({code:row.code,name:row.name,count:row.count===''?null:countValue(row.count)}));
  ...
  const importFile=(selected:File)=>void run(async()=>{
    ...
    const counts=new Map<string,string>();const codes=new Set(rows.map(row=>row.code.toUpperCase()));
    ...
    setRows(rows.map(row=>counts.has(row.code.toUpperCase())?{...row,count:counts.get(row.code.toUpperCase())!}:row));setDirty(true);setImportOpen(false);setMessage(`${counts.size} barang dimuat. Periksa selisih, lalu klik Simpan.`);
  });
  ...
  return <div className="adjustment-workspace opname-workspace" aria-busy={busy}>
    ...
    {list?<div className="adjustment-list">
      ...
      <div className="adjustment-body-wrap"><nav className="adjustment-side-tabs"><button title="Rincian Barang" className={!info?'active':''} onClick={()=>setInfo(false)}><FileText size={18}/></button><button title="Info Lainnya" className={info?'active':''} onClick={()=>setInfo(true)}><Info size={18}/></button></nav><section className="adjustment-body opname-body">
        ...
        <div className="opname-toolbar"><h2>Hasil Hitung Stok Opname <span>({rows.length} Barang)</span></h2><div className="adjustment-dropdown"><button className="adjustment-outline" disabled={!rows.length||busy||!canSave} onClick={()=>setImportOpen(!importOpen)}>Ambil <ChevronDown size={14}/></button>{importOpen&&<div className="adjustment-menu"><button onClick={()=>file.current?.click()}>Impor dari Excel</button><button onClick={template}>Unduh Template Excel</button></div>}</div><button className="adjustment-icon" title="Muat ulang stok acuan" disabled={busy||!warehouse} onClick={reload}><RefreshCw size={17}/></button><div className="opname-search"><input placeholder="Pencarian Barang…" aria-label="Pencarian Barang" value={search} onChange={e=>setSearch(e.target.value)}/><Search size={17}/></div><select aria-label="Filter hasil hitung" value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">Semua Barang</option><option value="blank">Belum Dihitung</option><option value="match">Cocok</option><option value="difference">Berselisih</option></select></div>

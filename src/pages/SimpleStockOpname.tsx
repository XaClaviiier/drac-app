...
type Document = {id:string;orderNumber:string;endDate:string;warehouseId:string;warehouseName:string;assignedUserName:string;notes:string;revision:number;entryMode:string;createdAt:string;rows?:Row[];result?:{status?:string;adjustmentId?:string;adjustmentNumber?:string;notes?:string}};
...
export default function SimpleStockOpname({legacy}:{legacy:ReactNode}) {
  ...
  const clear=()=>{setDocument(null);setRows([]);setCatalog([]);setAddSearch('');setAddOpen(false);setTableSearchOpen(false);setDate(localDateKey());setWarehouse('');setNotes('');setDirty(false);setSearch('');setFilter('all');setRequestKey(key());setInfo(false);setImportOpen(false);setMessage('');setCategoryOpen(false);};
  ...
  const takeCategory=(name:string)=>{const picked=categoryRows.filter(row=>row.categoryName===name&&!rows.some(old=>old.itemId===row.itemId));setRows(current=>[...current,...picked.map(row=>({...row,count:''}))]);setDirty(true);setCategoryOpen(false);setSearch('');setFilter('all');setMessage(`${picked.length} barang kategori ${name} ditambahkan.`);};
    ...
    {list?<div className="adjustment-list">
      ...
      <div className="adjustment-body-wrap"><nav className="adjustment-side-tabs"><button title="Rincian Barang" className={!info?'active':''} onClick={()=>setInfo(false)}><FileText size={18}/></button><button title="Info Lainnya" className={info?'active':''} onClick={()=>setInfo(true)}><Info size={18}/></button></nav><section className="adjustment-body opname-body">
        ...
        <div className="opname-toolbar opname-entry-toolbar">
          ...
          <div className="adjustment-dropdown"><button className="adjustment-outline" disabled={!catalog.length||busy||!canSave} onClick={()=>{setImportOpen(!importOpen);setAddOpen(false);}}>Ambil <ChevronDown size={14}/></button>{importOpen&&<div className="adjustment-menu"><button onClick={()=>{setCategoryOpen(true);setImportOpen(false);}}>Per Kategori</button><button onClick={()=>{setRows(current=>[...current,...catalog.filter(item=>!current.some(row=>row.itemId===item.itemId))]);setDirty(true);setImportOpen(false);setSearch('');setFilter('all');}}>Semua Barang</button><button onClick={()=>file.current?.click()}>Impor dari Excel</button><button onClick={template}>Unduh Template Excel</button></div>}</div>

...
test('menu SERVIS ORDER memakai standar tile Accurate sebagai default', () => {
  ...
  assert.match(layout, /isAccurateServiceMenu \|\| isAccurateLedgerMenu\s*\? "w-\[420px\]"/);
  assert.match(layout, /isAccurateServiceMenu \|\| isAccurateLedgerMenu\s*\? "grid-cols-\[repeat\(3,120px\)\] gap-2\.5"/);

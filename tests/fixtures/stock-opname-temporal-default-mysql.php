...
if (getenv('CI') !== 'true' || !getenv('MYSQL_CONTAINER')) throw new RuntimeException('Requires CI=true and MYSQL_CONTAINER');
$pdo=new PDO('mysql:host=127.0.0.1;dbname=drac_temporal_defaults;charset=utf8mb4','root','root',[
...
foreach (['standalone','runtime'] as $path) {
    foreach ($cases as $definition) {
        ...
        // Whole-second data keeps this test focused on definition serialization, not precision conversion.
        $pdo->exec("INSERT INTO stock_count_result_items(result_id,item_id,added_at) VALUES('R','I','2026-08-10 12:34:56')");

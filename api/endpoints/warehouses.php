...
switch ($method) {
    ...
    case 'PUT':
        ...
        try{
            ...
            $actor=$authorization['actor'];
            $current=$pdo->prepare("SELECT branch_id,is_default,is_system,is_active FROM warehouses WHERE id=? FOR UPDATE");$current->execute([$id]);$currentRow=$current->fetch();
            ...
            $pdo->commit();respondSuccess(null,'Gudang diperbarui');
        }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();respondError($error->getMessage(),transactionExceptionStatus($error,422));}
        break;
    case 'DELETE':
        ...
        try{
            ...
            $actor=$authorization['actor'];
            $stmt=$pdo->prepare("SELECT branch_id,is_default,is_system,is_active FROM warehouses WHERE id=? FOR UPDATE");$stmt->execute([$id]);$warehouse=$stmt->fetch();
            if(!$warehouse)throw new DomainException('Gudang tidak ditemukan',404);assertAccessibleBranch($pdo,$actor,(string)$warehouse['branch_id']);
            if($warehouse['is_default']||$warehouse['is_system'])throw new DomainException('Gudang utama/sistem tidak dapat dinonaktifkan',403);
            if($reason=$warehouseBlocker($pdo,$id))throw new DomainException($reason,409);
            $pdo->prepare("UPDATE warehouses SET is_active=0 WHERE id=?")->execute([$id]);
            $pdo->commit();respondSuccess(null,'Gudang dinonaktifkan');
        }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();respondError($error->getMessage(),transactionExceptionStatus($error,422));}
        break;

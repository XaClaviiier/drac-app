...
if ($method === 'GET') {
    ...
    $rows = array_values(array_filter($stmt->fetchAll(), static function($row) use ($pdo,$user) {
        return ($row['branch_id'] ?? null) === null || $row['branch_id'] === '' || !empty($user['is_owner']) || in_array($row['branch_id'], getUserBranchIds($pdo, $user['id']), true);
    }));
    foreach ($rows as &$row) {
...
if ($method === 'POST') {
    ...
    $stmt->execute([generateId(),$d['customerId'],$d['customerName']??'',$d['phone']??'',$d['templateType'],$d['messageText'],$d['vehicleId']??null,$d['vehicleInfo']??null,$d['workOrderId']??null,$d['workOrderNumber']??null,$d['invoiceId']??null,$d['invoiceNumber']??null,$branchId !== '' ? $branchId : null,$user['id']??null,$user['name']??$user['username']??null]);

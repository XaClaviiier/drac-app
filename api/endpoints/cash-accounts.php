...
case 'POST':
    ...
    $branchId=!empty($d['isCompanyWide'])?null:($d['branchId']??null);if(!$branchId&&!in_array($type,['bank','qris'],true))respondError('Cabang wajib dipilih untuk akun kas tunai',422);
    ...
    $ledger=$d['ledgerAccountId']??(['cash'=>'COA-1101','bank'=>'COA-1102','qris'=>'COA-1103'][$type]);
...
case 'PUT':
    if(!$id)respondError('ID wajib',422);$d=getInput();$type=$d['accountType']??'bank';$branchId=!empty($d['isCompanyWide'])?null:($d['branchId']??null);if($type==='cash'&&!$branchId)respondError('Cabang wajib dipilih untuk akun kas tunai',422);
...
case 'DELETE':
    if(!$id)respondError('ID wajib',422);$used=$pdo->prepare("SELECT (SELECT COUNT(*) FROM customer_payments WHERE account_id=?)+(SELECT COUNT(*) FROM branch_deposits WHERE source_account_id=? OR destination_account_id=?)+(SELECT COUNT(*) FROM branch_account_settings WHERE cash_account_id=? OR bank_account_id=? OR qris_account_id=? OR deposit_destination_account_id=?)");$used->execute([$id,$id,$id,$id,$id,$id,$id]);if((int)$used->fetchColumn()>0)respondError('Akun sudah digunakan pada transaksi atau pengaturan cabang. Nonaktifkan akun.',422);$pdo->prepare("DELETE FROM cash_accounts WHERE id=?")->execute([$id]);respondSuccess(null,'Akun dihapus');break;

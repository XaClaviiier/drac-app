...
ALTER TABLE `work_orders`
  MODIFY COLUMN `status` ENUM('Draft','Pengecekan','Proses','Selesai','Dibayar') NOT NULL DEFAULT 'Pengecekan';

UPDATE `work_orders` SET `status` = 'Pengecekan' WHERE `status` = 'Draft';

ALTER TABLE `work_orders`
  MODIFY COLUMN `status` ENUM('Pengecekan','Proses','Selesai','Dibayar') NOT NULL DEFAULT 'Pengecekan',
